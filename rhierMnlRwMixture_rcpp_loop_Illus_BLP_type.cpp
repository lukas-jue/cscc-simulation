...
// [[Rcpp::export]]
double llmnl_BC(vec const& betastar, vec const& y, mat const& X, int const& pr){
  ...
  int j = X.n_rows/n; //number of alternatives per choice task / number of players
  
  vec beta = startobeta(betastar);  //Thomas 2016-06-11
  
  vec beta_red = beta(span(1,nvar-1)); //select everything but budget (budget must be first entry of beta-coefficient)
  double budget = beta(0); //infer budget 
  vec X_BLP = zeros<vec>(X.n_cols);
  vec Xbeta = zeros(j);
  
  //mat Xbeta = X*beta_red; //should result in a (n*j x 1) vector
  ...
  for(int i = 0; i<n;i++){      
    for(int p = 0; p<j;p++){
      //Do denominator first...
      if (X(i*j+p,pr_cpp) <= budget) {
        X_BLP = vectorise(X(i*j+p,span::all));
        X_BLP(pr_cpp) = log(budget-X(i*j+p,pr_cpp)); 
        Xbeta(p) = sum(X_BLP % beta_red);
        denom[i] = denom[i] + exp(Xbeta(p));
      }
    }
    
    // Indicate if beta-vector violates data
    if(denom[i]==0){
      return(-1*datum::inf);
    } else {
      //Do numerator now...
      if (X(i*j+y[i]-1,pr_cpp) < budget) {
        xby[i] = Xbeta[y[i]-1];
      } else {
        //Insure zero likelihood if chosen alternative is outside budget
        return(-1*datum::inf);       
      }
    }
  }
...
mnlMetropOnceOut mnlMetropOnce_mod(vec const& y, mat const& X, vec const& oldbeta, 
                                   ...
                                   vec const& betabar, mat const& rootpi, int const& pr){ 
  ...
  int stay = 0;
  vec betac = oldbeta + s*trans(incroot)*as<vec>(rnorm(betabar.size()));
  double cll = llmnl_BC(betac,y,X,pr);
...
List rhierMnlRwMixture_rcpp_loop_mod(List const& lgtdata, int nvar_c, mat const& Z, vec const& deltabar, mat const& Ad, mat const& mustarbarc, mat const& Amu,
                                  ...
                                  vec ind_stay_temp, mat ind_stay, int ncomp, int const& pr){

//List rhierMnlRwMixture_rcpp_loop_mod(List const& lgtdata, int nvar_c, mat const& Z, vec const& deltabar, mat const& Ad, mat const& mustarbarc, mat const& Amu,
//                                  int const& nu, mat const& V, mat const& Gammabar, mat const& A_Gamma, int const& nu_Sigma, mat const& V_Sigma,
//                                  double s, int R, int keep, int nprint, bool drawdelta, mat olddelta,  vec const& a, vec oldprob, mat oldbetas, vec ind,
//                                  mat const& betas_tuning, vec const& betastarpooled, vec ind_stay_temp, mat ind_stay, int ncomp, int const& pr){
  ...
  for (int rep = 0; rep<R; rep++){
    ...
    //loop over all LGT equations drawing beta_i | ind[i],z[i,],mu[ind[i]],rooti[ind[i]]
      for(int lgt = 0; lgt<nlgt; lgt++){
        ...
        if (rep == 0) oldll[lgt] = llmnl_BC(vectorise(oldbetas(lgt,span::all)),lgtdata_vector[lgt].y,lgtdata_vector[lgt].X, pr);
        ...
        metropout_struct = mnlMetropOnce_mod(lgtdata_vector[lgt].y,lgtdata_vector[lgt].X,vectorise(oldbetas(lgt,span::all)),
                                         oldll[lgt],s,incroot,betabar,rootpi,pr);
...
List drawCompsFromLabels(mat const& y, int nvar_c, mat const& mustarbarc, mat const& Amu, int nu, mat const& V, mat const& Gammabar, mat const& A_Gamma,  
  ...
  for(int k = 0; k<ncomp; k++){
      ...
      // Vbetastarc
      mat V_inv_r = solve(trimatu(chol(V)), eye(V.n_rows,V.n_cols));
      mat V_inv = V_inv_r*trans(V_inv_r);
